...
```{r}
#nei = as.character( df$neighbourhood_cleansed )
#nei[nei != "Alki" & nei != "Arbor Heights" & nei != "Belltown" & nei != "Bitter Lake" & nei != "Briarcliff" & nei != "Broadview"] <- "Other"
#summary(as.factor(nei))
...
```{r}
corrplot(cor(df[,c("price", "host_response_rate", "accommodates", "review_scores_rating", "reviews_per_month", "minimum_nights","availability_365")]), method="circle")
```

price ~  
"host_response_rate", "neighbourhood_cleansed",  "room_type", "accommodates",  "availability_365",  "review_scores_rating", "reviews_per_month", "instant_bookable", "cancellation_policy", "minimum_nights", "parking", "washer", "checkin24", "Pets_Allowed", "review_rating".


```{r}
hist(df$host_response_rate, breaks=50)
hist(df$accommodates, breaks=50)
hist(df$review_scores_rating, breaks=50)
hist(df$reviews_per_month, breaks=50)
hist(df$minimum_nights, breaks=50)
hist(df$availability_365, breaks=50)

#histogram categorical variable 
plot(df$neighbourhood_cleansed, main="Neighbourhood")
...
```{r}
corrplot(cor(df[,c("price", "host_response_rate", "accommodates", "review_scores_rating", "reviews_per_month", "minimum_nights")]), method="circle")
```
...
hist(df$review_scores_rating, breaks=50, main="review_scores_rating")
...
plot(df$Pets_Allowed,main="Pets Allowed")
<<<<<<< HEAD
# removed plot(df$property_type,main="Property Type")
=======
>>>>>>> 45547a58d84f562a4d7199a82471257a4d6e5389
plot(df$instant_bookable,main="Instant Bookable")
```
...
```{r}
fit1 = lm(price ~  host_response_rate + neighbourhood_group + room_type+accommodates + availability_365 + review_scores_rating+ reviews_per_month+ instant_bookable + cancellation_policy + minimum_nights + parking + washer + checkin24 + Pets_Allowed + review_rating, data=df)
summary(fit1)
```

```{r}
fit2 = lm(log(price) ~  host_response_rate + neighbourhood_cleansed + room_type+log(accommodates) + availability_365 + review_scores_rating + reviews_per_month + instant_bookable + cancellation_policy + minimum_nights + parking + washer + checkin24 + Pets_Allowed + review_rating, data=df)
summary(fit2)
```

```{r}
model = lm(price~ host_response_rate + neighbourhood_cleansed + room_type+accommodates+availability_365+review_scores_rating+reviews_per_month+ instant_bookable+cancellation_policy+minimum_nights+parking+washer+checkin24+Pets_Allowed+review_rating, data = df)
...
```{r}
<<<<<<< HEAD
summary(lm(price ~ as.factor(neighbourhood_group),data = df))

=======
summary(lm(price ~ as.factor(neighbourhood_cleansed),data = df))
>>>>>>> 45547a58d84f562a4d7199a82471257a4d6e5389
```
...
sample.a = sample(nrow(df), 0.75*nrow(df))
train.a = df[sample.a,]
test.a = df[-sample.a,]
library(randomForest)
model.a = randomForest(price~host_response_rate +neighbourhood_group +room_type+bathrooms+bedrooms +availability_365+number_of_reviews+minimum_nights, data=train.a,mtry=3,importance=TRUE, na.action=na.omit)
model.a
#prediction = predict(model, newdata=test)
#table(prediction, test$DEFAULT)
