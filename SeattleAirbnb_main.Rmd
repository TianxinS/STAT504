...
```{r}
hist(log(df$host_response_rate), breaks=50)
```
...
#plot(df$property_type)

#names=df %>%
#group_by(property_type) %>% summarise(n = NROW('availability_365'))

#barplot(df, main = "Math by SES", xlab = "SES", ylab = "Mean Math Score", names.arg = names)
```
